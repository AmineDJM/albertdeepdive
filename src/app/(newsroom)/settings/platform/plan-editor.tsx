    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Changes apply to every workspace on this plan, and to the public pricing page. Key: <span className="font-mono">{plan.key}</span>
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="pl-trial">Trial days</Label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="pl-monthly">Monthly ({plan.currency})</Label>
              <Input id="pl-monthly" value={monthly} onChange={(e) => setMonthly(e.target.value)} inputMode="decimal" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="pl-yearly">Yearly ({plan.currency})</Label>
              <Input id="pl-yearly" value={yearly} onChange={(e) => setYearly(e.target.value)} inputMode="decimal" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="pl-sm">Stripe monthly price</Label>
              <Input id="pl-sm" value={monthlyPriceId} onChange={(e) => setMonthlyPriceId(e.target.value)} placeholder="price_…" className="font-mono text-xs" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="pl-sy">Stripe yearly price</Label>
              <Input id="pl-sy" value={yearlyPriceId} onChange={(e) => setYearlyPriceId(e.target.value)} placeholder="price_…" className="font-mono text-xs" />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="pl-highlights">Highlights (one per line)</Label>
            <Textarea id="pl-highlights" rows={5} value={highlights} onChange={(e) => setHighlights(e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="pl-entitlements">Entitlements</Label>
            ...
            <p className="text-xs text-muted-foreground">
              <span className="font-mono">null</span> means unlimited; <span className="font-mono">0</span> means none.
            </p>
            {jsonError ? <p className="text-xs text-destructive">{jsonError}</p> : null}
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
              <Label htmlFor="pl-public">Shown on the pricing page</Label>
              <Switch id="pl-public" checked={isPublic} onCheckedChange={setIsPublic} />
            </div>
            <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
              <Label htmlFor="pl-featured">Marked &ldquo;most popular&rdquo;</Label>
