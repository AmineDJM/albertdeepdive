...
export default async function EditionLayout({ children, params }: { children: React.ReactNode; params: Promise<{ editionId: string }> }) {
  ...
  const tabs = EDITION_TABS.filter((t) => !t.permission || roleHasPermission(user.role, t.permission)).map((t) => ({ href: `/editions/${edition.id}${t.slug ? `/${t.slug}` : ""}`, label: translate(t.label), exact: t.slug === "" }));
  return (
