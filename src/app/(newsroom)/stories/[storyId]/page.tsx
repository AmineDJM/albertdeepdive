    ...
    <>
      ...
      <PageBody className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-5">
          ...
          <section>
            ...
            {article && article.status !== "EMPTY" ? (
              <div className="rounded-lg border border-border bg-card p-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-2xs text-muted-foreground">
                    {article.wordCount} words · revision {article.currentRevision}
              ...
              <div className="rounded-lg border border-dashed border-border p-4 text-center">
                ...
                <p className="mt-2 text-xs text-muted-foreground">No draft yet. The AI can write one from the fact sheet below; you stay in control of every sentence.</p>
              </div>
            )}
          </section>

          <section>
            <SectionTitle>Fact sheet ({story.facts.length})</SectionTitle>
          ...
          {story.quotes.length ? (
            <section>
              <SectionTitle>Quotes ({story.quotes.length})</SectionTitle>
                  ...
                  <li key={q.id} className="rounded-lg border border-border bg-card p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-serif text-[13px] italic">&ldquo;{q.text}&rdquo;</p>
                      ...
                      {q.isPullQuoteCandidate ? <Badge variant="brand">Pull quote</Badge> : null}
                    </div>
                  </li>
                ))}
              </ul>
          ...
          {story.media.length ? (
            <section>
              <SectionTitle>Media ({story.media.length})</SectionTitle>
          ...
          {story.bdd ? (
            <section>
              <SectionTitle>Business Deep Dive</SectionTitle>
        ...
        <aside className="space-y-5">
          <section className="rounded-lg border border-border bg-card p-3">
            <SectionTitle>Sources ({submissions.length})</SectionTitle>
            <ul className="space-y-2">
                ...
                <li key={sub.id}>
                  <Link href={`/editions/${edition.id}/inbox?submission=${sub.id}`} className="block rounded-md border border-border px-2.5 py-2 hover:border-brand/50">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {i === 0 ? <Badge variant="brand">Primary</Badge> : null}
                    </div>
                    ...
                    <p className="mt-0.5 text-2xs text-muted-foreground">
                      {sub.campuses.map((c) => c.campus.name).join(", ") || "School-wide"} · {sub.mediaAssets.length} photo{sub.mediaAssets.length === 1 ? "" : "s"}
                    </p>
                  </Link>
                </li>
              ))}
            </ul>
            {cluster ? <p className="mt-2 text-2xs text-muted-foreground">Cluster: {cluster.title}</p> : null}
          </section>
          ...
          <section className="rounded-lg border border-border bg-card p-3">
            ...
            <dl className="space-y-1.5 text-xs">
              ...
              <div className="flex justify-between gap-2">
                ...
                <dd className="font-medium">{enumLabel(story.targetLength)}</dd>
              </div>
              <div className="flex justify-between gap-2">
            ...
            {story.aiScores && Object.keys(story.aiScores).length ? (
              <>
                ...
                <div className="label-caps mb-1">AI scoring</div>
                <ul className="space-y-1">
                  {Object.entries(story.aiScores)
                    .filter(([k]) => k !== "total")
                    .map(([k, v]) => (
                      <li key={k} className="flex items-center gap-2 text-2xs">
                        <span className="w-28 shrink-0 truncate text-muted-foreground">{enumLabel(k.replace(/([A-Z])/g, " $1"))}</span>
          ...
          {story.people.length || story.organisations.length ? (
            <section className="rounded-lg border border-border bg-card p-3">
              <SectionTitle>People &amp; organisations</SectionTitle>
              {story.people.length ? (
                <div className="mb-2">
                  ...
                  <ul className="space-y-0.5">
                    {story.people.map((p) => (
                      <li key={`${p.personId}-${p.role}`} className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <Badge variant="muted">{enumLabel(p.role)}</Badge>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {story.organisations.length ? (
                <div>
                  <div className="label-caps mb-1">Organisations</div>
                  <ul className="space-y-0.5">
                    {story.organisations.map((o) => (
                      <li key={`${o.organisationId}-${o.role}`} className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <Badge variant="muted">{enumLabel(o.role)}</Badge>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div key={e.id} className="text-xs">
                  ...
                  {e.signupUrl ? (
                    <a href={e.signupUrl} target="_blank" rel="noreferrer" className="mt-0.5 inline-flex items-center gap-1 text-brand hover:underline">
                      Sign-up link <ExternalLink className="size-3" />
                    </a>
                  ) : null}
                </div>
              ))}
