...
export async function signInAction(_prev: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  if (!valid) {
    ...
    return fail(tr("Incorrect email or password."));
  }
  if (!NEWSROOM_ROLES.includes(user.role)) return fail(tr("Your account can submit stories but cannot access the newsroom."));
