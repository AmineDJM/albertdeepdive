...
import { InvoiceControls } from "../../billing/invoice-controls";
import { SubscriptionControls } from "../../billing/subscription-controls";
  ...
  return (
    <>
      ...
      <PageBody className="space-y-6">
        <Link href="/admin/organizations" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
        ...
        <section>
          ...
          <DataTable
            ...
            onRowHref={(member) => `/admin/users/${member.userId}`}
        ...
        <section>
          <SectionTitle action={<Link href={`/admin/audit?workspace=${id}&days=30`} className="text-xs text-muted-foreground hover:text-foreground">{tr("All logs")}</Link>}>{tr("Activity, 30 days")}</SectionTitle>
