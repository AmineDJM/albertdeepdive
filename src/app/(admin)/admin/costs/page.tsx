...
export default async function PlatformCostsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!hasPermission(user, "settings:manage")) {
    return (
      <>
        <PageHeader title={tr("Usage & costs")} />
        <PageBody>
      ...
      <PageHeader
        ...
        actions={
          <Button variant="outline" size="sm" asChild>
            <a href={`/admin/costs/export?days=${days}`}>
      ...
      <PageBody className="space-y-6">
        ...
        <section>
          ...
          <DataTable
            ...
            onRowHref={(row) => `/admin/organizations/${row.organizationId}`}
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <section>
            ...
            <DataTable
              ...
              onRowHref={(row) => `/admin/users/${row.userId}`}
