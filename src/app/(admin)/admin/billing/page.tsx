...
export default async function PlatformPaymentsPage() {
  ...
  const subscriptionColumns = [
    {
      ...
      cell: (sub: SubscriptionRow) => (
        <Link href={`/admin/organizations/${sub.organizationId}`} className="font-medium hover:underline" data-no-row-link>
          {sub.name}
        </Link>
      ),
    },
  ...
  const invoiceColumns = [
    {
      ...
      cell: (invoice: InvoiceRow) => (
        <span className="flex flex-col">
          ...
          <span className="text-2xs text-muted-foreground">{invoice.workspace ? <Link href={`/admin/organizations/${invoice.workspace.id}`} className="hover:underline" data-no-row-link>{invoice.workspace.name}</Link> : (invoice.customerEmail ?? tr("unknown customer"))}</span>
        </span>
      ),
    },
  ...
  return (
    <>
      <PageHeader title={tr("Billing")} description={overview.configured ? (overview.account ? `${tr("Stripe account")} ${overview.account}${overview.livemode === false ? ` · ${tr("test mode")}` : ""}` : tr("Stripe is connected.")) : tr("Payments are not connected: the figures below come from the plans customers are on, and nothing can be charged.")} />
      <PageBody className="space-y-6">
        ...
        {!overview.configured ? (
          <p className="text-xs text-muted-foreground">
            {tr("Connect Stripe under")}{" "}<Link href="/admin/providers" className="font-medium text-foreground hover:underline">{tr("Integrations")}</Link>{" "}{tr("to see invoices, retry declined cards and send reminders from here.")}</p>
        ) : null}
        ...
        <section>
          ...
          <DataTable rows={overview.subscriptions} rowKey={(sub) => sub.organizationId} onRowHref={(sub) => `/admin/organizations/${sub.organizationId}`} empty={{ title: tr("No workspaces"), description: tr("The first one is created by onboarding."), icon: Wallet }} columns={subscriptionColumns} />
        </section>
