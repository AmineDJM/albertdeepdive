...
import { auditSpec, composeSpec } from "@/lib/creative/compose";
...
export async function setBrief(input: { packId: string; brief: unknown; actorId?: string | null }): Promise<CreativePack> {
  ...
  const spec = composeSpec(brief, compileBrandSystem(record.system), { brandVersion: record.id });
  const problems = auditSpec(spec);
  ...
  const [updated] = await db
    ...
    .set({
      ...
      status: problems.length ? "FAILED" : "COMPOSING",
      error: problems.length ? problems.map((p) => `frame ${p.frame + 1}: ${p.problem}`).join("; ") : null,
      updatedAt: new Date(),
    })
  ...
  await audit({
    ...
    userId: input.actorId ?? null,
    metadata: { frames: brief.frames.length, fingerprint: spec.fingerprint, problems: problems.length },
  });
  return updated;
}
