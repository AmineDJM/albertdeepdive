...
export const INTEGRATIONS: IntegrationDefinition[] = [
  ...
  },
  {
    ...
    category: "media",
    summary: "Realistic scenes, people and products, holding several references at once: the picture model routed to for photographs and high-end campaign imagery.",
    docsUrl: "https://aistudio.google.com/apikey",
    docsLabel: "Google AI Studio → API keys",
    primaryField: "apiKey",
    testable: true,
    whenMissing: "Realistic pictures fall to the next connected model; identity-preserving edits have one fewer option.",
    fields: [
      { key: "apiKey", label: "API key", kind: "secret", placeholder: "AIza…", required: true, envVar: "GEMINI_API_KEY" },
      { key: "imageModel", label: "Image model", kind: "text", placeholder: "gemini-3-pro-image-preview", envVar: "GOOGLE_IMAGE_MODEL", help: "The Gemini image model to ask. Leave empty for the registry's default." },
      { key: "baseUrl", label: "Base URL", kind: "url", placeholder: "https://generativelanguage.googleapis.com/v1beta", envVar: "GOOGLE_AI_BASE_URL" },
    ],
  },
  {
    key: "recraft",
    name: "Recraft",
    category: "media",
    summary: "Illustration, icons and vector artwork: branded visual systems that scale.",
    docsUrl: "https://www.recraft.ai/profile/api",
    docsLabel: "Recraft → API",
    primaryField: "apiKey",
    testable: true,
    whenMissing: "Illustrations and icons fall to the next connected model; vector output is unavailable.",
    fields: [
      { key: "apiKey", label: "API key", kind: "secret", required: true, envVar: "RECRAFT_API_KEY" },
      { key: "model", label: "Model", kind: "text", placeholder: "recraftv3", envVar: "RECRAFT_MODEL" },
    ],
  },
  {
    key: "ideogram",
    name: "Ideogram",
    category: "media",
    summary: "Posters and campaign graphics where lettering is part of the artwork. The final words are still set by Briefly's renderer.",
    docsUrl: "https://ideogram.ai/manage-api",
    docsLabel: "Ideogram → API",
    primaryField: "apiKey",
    testable: false,
    whenMissing: "Poster-style pictures fall to the next connected model.",
    fields: [
      { key: "apiKey", label: "API key", kind: "secret", required: true, envVar: "IDEOGRAM_API_KEY" },
      { key: "model", label: "Model", kind: "text", placeholder: "ideogram-v3", envVar: "IDEOGRAM_MODEL" },
    ],
  },
  {
    key: "images",
    ...
    fields: [
      ...
      { key: "openaiModel", label: "OpenAI image model", kind: "text", placeholder: "gpt-image-1", envVar: "OPENAI_IMAGE_MODEL", help: "The model behind “sunburst”. Change it here when a newer one is available to the key." },
      { key: "qaPassAt", label: "Keep a picture from", kind: "text", placeholder: "0.7", envVar: "IMAGE_QA_PASS_AT", help: "Check score, 0–1. Below it the job tries again; well below it, another model." },
      { key: "qaRetryBelow", label: "Give up on a model below", kind: "text", placeholder: "0.45", envVar: "IMAGE_QA_RETRY_BELOW" },
      { key: "maxRetries", label: "Retries per model", kind: "text", placeholder: "2", envVar: "IMAGE_MAX_RETRIES" },
      { key: "defaultVariations", label: "Candidates by default", kind: "text", placeholder: "auto", envVar: "IMAGE_DEFAULT_VARIATIONS", help: "Leave empty to let the plan decide: one for an edit, two for an uncertain new scene." },
      { key: "visionQa", label: "Look at results with the model", kind: "text", placeholder: "true", envVar: "IMAGE_VISION_QA", help: "Sends before/after to the connected vision model for faces, hands, artifacts and unwanted changes. Costs a little per picture." },
      { key: "showRouting", label: "Show routing to customers", kind: "text", placeholder: "false", envVar: "IMAGE_SHOW_ROUTING", help: "Platform staff always see which model made a picture; customers only when this says true." },
    ],
  },
  {
    key: "browserbase",
