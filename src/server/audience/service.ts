...
export async function listRecipients(filters: AudienceFilters = {}) {
  const where = await scoped(
    s.audienceRecipients.organizationId,
...
export async function getRecipient(id: string) {
  const row = await guardTenant(await db.query.audienceRecipients.findFirst({ where: eq(s.audienceRecipients.id, id), with: { campus: true } }), "Recipient");
...
export async function createRecipient(raw: z.input<typeof audienceInputSchema>, userId?: string | null, source = "manual") {
  ...
  const existing = await db.query.audienceRecipients.findFirst({ where: await scoped(s.audienceRecipients.organizationId, eq(s.audienceRecipients.email, input.email)) });
  if (existing) throw new ValidationError("A recipient with this email already exists", { email: ["Already used"] });
  const [row] = await db.insert(s.audienceRecipients).values(await stampTenant({ ...input, source })).returning();
...
export async function setRecipientActive(id: string, isActive: boolean, userId?: string | null) {
  const [row] = await db.update(s.audienceRecipients).set({ isActive }).where(await scoped(s.audienceRecipients.organizationId, eq(s.audienceRecipients.id, id))).returning();
  if (!row) throw new NotFoundError("Recipient");
...
export async function deleteRecipient(id: string, userId?: string | null) {
  const [row] = await db.delete(s.audienceRecipients).where(await scoped(s.audienceRecipients.organizationId, eq(s.audienceRecipients.id, id))).returning();
  if (!row) throw new NotFoundError("Recipient");
...
export async function audienceFacets() {
  const [totals] = await db
    ...
    .where(await scoped(s.audienceRecipients.organizationId));
  const segmentRows = await db.select({ segment: s.audienceRecipients.segment, n: count() }).from(s.audienceRecipients).where(await scoped(s.audienceRecipients.organizationId)).groupBy(s.audienceRecipients.segment);
