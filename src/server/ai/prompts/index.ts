...
- When they quote the words they want gone — "enlève ce passage", "remove this bit", followed by the text — that is remove_passage with the passage copied exactly as they gave it, never shorten_article. A word count says how much goes, never which words: asked to lose forty words, a copy editor may perfectly well keep the paragraph they pointed at and cut somewhere else. Copy their quotation verbatim; if the words are not in the issue the operation will refuse rather than guess, which is the right outcome.
