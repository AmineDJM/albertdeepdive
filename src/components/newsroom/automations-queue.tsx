  ...
  return (
    <>
      <DataTable
        ...
        rowKey={(j) => j.id}
        dense
        empty={{ title: "The queue is empty", description: "Background work (invitations, AI processing, media, exports, emails) appears here while it runs and stays visible after it finishes.", icon: ListRestart }}
        columns={[
          { key: "type", header: "Job", cell: (j) => (
          ...
          { key: "attempts", header: "Attempts", cell: (j) => <span className="tabular text-xs">{j.attempts}/{j.maxAttempts}</span>, align: "right" },
          { key: "priority", header: "Priority", cell: (j) => <span className="tabular text-xs text-muted-foreground">{j.priority}</span>, align: "right" },
          { key: "runAt", header: "Run at", cell: (j) => <span className="text-xs text-muted-foreground" title={j.display.runAt}>{j.display.runAtAgo}</span> },
          { key: "duration", header: "Duration", cell: (j) => <span className="tabular text-xs text-muted-foreground">{duration(j.durationMs)}</span>, align: "right" },
          { key: "error", header: "Last error", cell: (j) => (j.lastError ? <span className="line-clamp-1 max-w-[22rem] text-2xs text-destructive" title={j.lastError}>{j.lastError}</span> : <span className="text-2xs text-muted-foreground">—</span>) },
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2 font-mono">
                  ...
                  {inspected.attempts >= inspected.maxAttempts && inspected.status === "DEAD" ? <Badge variant="destructive">Dead letter</Badge> : null}
                </DialogTitle>
                <DialogDescription>
                  Attempt {inspected.attempts} of {inspected.maxAttempts} · created {inspected.display.createdAt} · run at {inspected.display.runAt}
              ...
              <div className="space-y-3">
                ...
                <Json label="Payload" value={inspected.payload} />
                {inspected.result ? <Json label="Result" value={inspected.result} /> : null}
    ...
    <div className="flex items-center gap-2">
      {canManage && retryable > 0 ? (
        <Button size="sm" variant="outline" loading={pending && action === "retry"} onClick={() => run("retry")}>
          <RotateCcw /> Retry {retryable} failed
        </Button>
      ) : null}
      {canManage ? (
