...
export function ProcessingButton({ editionId, unprocessed, label = "Run AI processing", variant = "default" }: { editionId: string; unprocessed: number; label?: string; variant?: "default" | "outline" | "brand" }) {
  ...
  function run() {
    startTransition(async () => {
      ...
      if (res.ok) {
        toast.success(tr("AI processing finished"), { description: res.message });
        setOpen(false);
        router.refresh();
      } else {
  ...
  return (
    <>
      ...
      <AlertDialog open={open} onOpenChange={(o) => !pending && setOpen(o)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{tr("Run the AI pipeline for this edition?")}</AlertDialogTitle>
            <AlertDialogDescription>
              ...
              {tr("Submissions are then grouped into story clusters with a sourced fact sheet, scored and turned into story candidates. Nothing is written or published without you.")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
