...
export function AiPanel({
  ...
  function describe() {
    startTransition(async () => {
      ...
      if (!res.ok) {
        toast.error(tr("Could not describe this image"), { description: res.error });
        return;
      }
    ...
    <div className="space-y-2.5">
      {description ? (
        ...
        <p className="text-muted-foreground text-xs">
          {tr("No AI description yet. The description feeds alt text, search and the media recommendations for stories — people are never named.")}</p>
      )}
      {tags.length ? (
