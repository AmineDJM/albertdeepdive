    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {value ? (
          ...
          <Button><Plus /> Add recipient</Button>
        )}
      </DialogTrigger>
      <DialogContent size="lg">
        <DialogHeader>
          <DialogTitle>{value ? `Edit ${value.firstName} ${value.lastName}`.trim() : "Add a recipient"}</DialogTitle>
          <DialogDescription>Recipients receive the finished magazine. This directory is separate from the contributor pool.</DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <NativeSelect value={form.segment} onChange={(e) => set("segment", e.target.value as RecipientEditorValue["segment"])}>
              {AUDIENCE_SEGMENTS.map((seg) => <option key={seg} value={seg}>{enumLabel(seg)}</option>)}
            </NativeSelect>
          </div>
          <div className="space-y-1.5">
            <Label>Campus</Label>
          ...
          <div className="col-span-2 space-y-1.5"><Label>Organisation</Label><Input value={form.organisation ?? ""} onChange={(e) => set("organisation", e.target.value || null)} placeholder="e.g. partner company, association" /></div>
          <div className="col-span-2 space-y-1.5"><Label>Tags</Label><Input value={form.tags.join(", ")} onChange={(e) => set("tags", e.target.value.split(",").map((t) => t.trim()).filter(Boolean))} placeholder="vip, print, newsletter" /></div>
          ...
          <label className="col-span-2 flex items-center justify-between rounded-md border px-3 py-2 text-[13px]"><span>Active — receives the magazine</span><Switch checked={form.isActive} onCheckedChange={(v) => set("isActive", v)} /></label>
        </div>
        <DialogFooter>
