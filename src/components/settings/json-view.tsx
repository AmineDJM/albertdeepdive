...
export function JsonView({ value, className, maxHeight = 320 }: { value: unknown; className?: string; maxHeight?: number }) {
  let text: string;
  try {
  ...
  if (text === undefined || text === "undefined" || text === "null" || text === "{}" || text === "[]") {
    return <div className={cn("rounded-md border border-dashed border-border px-3 py-2 text-2xs text-muted-foreground", className)}>Empty</div>;
  }
  return (
