...
describe("utils", () => {
  it("slugifies accents and punctuation", () => {
    expect(slugify("Albert's Deep Dive — Special issue N°1 (May 2025)")).toBe("albert-s-deep-dive-special-issue-n-1-may-2025");
...
describe("ai helpers", () => {
  it("renders template variables", () => {
    expect(renderTemplate("Hello {{name}} from {{campus}} ({{missing}})", { name: "Milan", campus: "Paris" })).toBe("Hello Milan from Paris (—)");
    expect(renderTemplate("{{list}}", { list: ["a", "b"] })).toBe("a, b");
  });
  it("builds strict JSON schemas for OpenAI structured outputs", () => {
    const schema = z.object({ title: z.string().min(1), tags: z.array(z.string()), score: z.number().min(0).max(1).nullable(), nested: z.object({ ok: z.boolean() }) });
    const json = toStrictJsonSchema(schema) as { additionalProperties: boolean; required: string[]; properties: Record<string, Record<string, unknown>> };
    expect(json.additionalProperties).toBe(false);
    expect(json.required).toEqual(["title", "tags", "score", "nested"]);
    expect(json.properties.title.minLength).toBeUndefined();
    expect((json.properties.nested as { additionalProperties: boolean }).additionalProperties).toBe(false);
  });
