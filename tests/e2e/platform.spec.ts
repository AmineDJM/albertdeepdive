...
test.describe("platform staff", () => {
  test("land on the console and open a customer only on purpose", async ({ page }) => {
    ...
    // The console is its own surface: a dark shell, its own column, nothing of a workspace.
    const console_ = page.getByRole("navigation", { name: "Admin" });
    await expect(console_.getByRole("link", { name: "Organizations" })).toBeVisible();
    ...
    await expect(page).toHaveURL(/\/admin$/);
    // The old door still opens onto the new console.
  ...
  test("read a customer, a person, the bill and the money", async ({ page }) => {
    ...
    // The sections the brief asked for, each a real page with a title of its own.
    for (const [path, title] of [["/admin/plans", "Plans & pricing"], ["/admin/jobs", "Jobs"], ["/admin/flags", "Feature flags"], ["/admin/support", "Support"], ["/admin/system", "System"], ["/admin/audit", "Audit log"]] as const) {
    ...
    await expect(page.getByRole("heading", { name: "Albert School" })).toBeVisible();
    ...
    await page.goto("/admin/costs?days=7");
    await expect(page.getByRole("heading", { level: 1 })).toHaveText("Usage & costs");
